...
export default async function ComparePage({searchParams}:{searchParams:Promise<{ids?:string}>}){const {ids}=await searchParams;const data=await catalog();const known=[...data.machines,...data.referenceMachines];const valid=[...new Set((typeof ids==='string'?ids:'').split(','))].filter(id=>known.some(m=>m.id===id)).slice(0,3);return <CatalogApp {...data} mode="compare" initialCompare={valid}/>;}
